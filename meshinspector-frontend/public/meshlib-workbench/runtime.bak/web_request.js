...
var web_req_ensure_formdata = function (ctxId) {
    create_web_ctx_if_needed(ctxId);
    if (web_req_ctxs[ctxId].formdata == null)
        web_req_ctxs[ctxId].formdata = new FormData();
}

var web_req_add_formdata_text = function (value, name, ctxId) {
    web_req_ensure_formdata(ctxId);
    web_req_ctxs[ctxId].formdata.append(name, value);
}

var web_req_add_formdata_file = function (path, contentType, name, fileName, ctxId) {
...
var web_req_add_formdata = web_req_add_formdata_file;
...
var web_req_build_url = function (url, ctxId) {
    for (var i = 0; i < web_req_ctxs[ctxId].params.length; i++) {
        if (i == 0)
            ...
            url += "&";
        url += encodeURIComponent(web_req_ctxs[ctxId].params[i].key) + "=" + encodeURIComponent(web_req_ctxs[ctxId].params[i].value);
    }
    return url;
}
...
var web_req_async_download = function (url, outputPath, ctxId) {
    ...
    if (web_req_ctxs[ctxId].method != "GET" && web_req_ctxs[ctxId].method != "HEAD") {
        options.body = payload ?? web_req_ctxs[ctxId].formdata ?? web_req_ctxs[ctxId].body;
    }
