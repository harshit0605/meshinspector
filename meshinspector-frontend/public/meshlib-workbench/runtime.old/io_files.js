...
var save_file = function (filename) {
  var checkPath = function (filename) {
    ...
    if (window.MeshInspectorWorkbenchBridge?.commitSavedFile) {
      void window.MeshInspectorWorkbenchBridge.commitSavedFile(filename, content, {
        metadata: {
          save_dialog_filename: filename,
        },
      });
    }
